...
MOCK_GMAIL_MULTIPART_MESSAGE = {
	...
	'payload': {
		...
		'parts': [
			{
				'mimeType': 'text/plain',
				'body': {'data': base64.urlsafe_b64encode('プレーンテキスト部分です。'.encode()).decode()},
			},
			{
				'mimeType': 'text/html',
				'body': {
					'data': base64.urlsafe_b64encode('<html><body>HTML部分です。</body></html>'.encode()).decode()
				},
			},
			{
				'mimeType': 'text/plain',
				'body': {'data': base64.urlsafe_b64encode(' 追加のプレーンテキスト。'.encode()).decode()},
			},
		],
	},
}
...
# Gmail API レスポンスモック
MOCK_GMAIL_LIST_RESPONSE = {'messages': [{'id': 'mock_message_id_12345'}, {'id': 'mock_message_id_67890'}]}

MOCK_GMAIL_NO_MESSAGES_RESPONSE: dict[str, list[Any]] = {'messages': []}

# LINE API モックデータ
MOCK_LINE_SUCCESS_RESPONSE = {'message': 'ok'}

MOCK_LINE_ERROR_RESPONSE = {'message': 'Invalid channel access token'}

# Slack API モックデータ
MOCK_SLACK_SUCCESS_RESPONSE = {'ok': True, 'channel': 'C1234567890', 'ts': '1234567890.123456'}

MOCK_SLACK_ERROR_RESPONSE = {'ok': False, 'error': 'channel_not_found'}
...
def get_mock_env_vars() -> dict[str, str]:
	...
	mock_token = base64.b64encode(pickle.dumps(mock_creds)).decode('utf-8')
...
def create_test_email_content(
	subject: str = 'テストメール', from_email: str = 'test@example.com', body: str = 'テスト本文'
) -> dict[str, str]:
	"""カスタムテスト用メールコンテンツを作成"""
	return {'id': 'custom_test_id', 'subject': subject, 'from': from_email, 'body': body}

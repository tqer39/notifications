...
class TestGmailNotifier:
	...
	def test_init(self, mock_credentials, mock_build):
		...
		mock_build.assert_called_once_with('gmail', 'v1', credentials=mock_credentials.from_service_account_info.return_value)

	@patch('src.gmail_notifier.build')
	@patch('src.gmail_notifier.service_account.Credentials')
	def test_get_unread_fts_emails_no_messages(self, mock_credentials, mock_build):
		...
		payload = {
			'parts': [
				...
				},
				{
					'mimeType': 'text/html',
					'body': {
						'data': base64.urlsafe_b64encode(b'<html>Part 2</html>').decode()
					}
				},
				{
					'mimeType': 'text/plain',
					'body': {
						'data': base64.urlsafe_b64encode(b' Part 3').decode()
					}
				}
			]
		}
